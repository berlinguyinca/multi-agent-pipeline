...
- Tests are mandatory verification evidence for software-delivery lanes. Database/service dependencies should run in isolated Docker-backed test services or project-provided test containers, never against host databases or shared main-system services.
- For PubChem downloader/synchronizer workflows, QA should require proof that the generated tool processed 1000 PubChem records into Markdown in an isolated output folder, or a deterministic 1000-record fixture plus a live-network blocker.
