<!-- generated by scripts/generate_resolver_docs.py; edit src/chem_evidence/routing_catalog.py -->

# EPA CompTox / DSSTox

Namespace: `comptox`

## What this supports

Chemlake resolves EPA CompTox / DSSTox queries from local lake data and returns ranked compound matches. Request-time resolution is offline; this route never calls an upstream service.

## Accepted inputs

`DTXSID5020108, DTXCID..., comptox:DTXSID...`

## Local artifact requirements

- `compound_identity.jsonl/parquet or xrefs.jsonl/parquet`

The artifact must contain values that can be normalized into this namespace and joined to a Chemlake compound identity.

## Resolver route

- Route: **Xref index**
- Behavior: Source/database identifiers normalized by namespace and joined to compound identity.
- Ranking: candidates are merged with other matching routes, then scored with ambiguity-aware confidence components when `--confidence` is requested.

## Outputs

comptox plus all compound outputs.

When `--to all` is requested, this route can also return linked names, synonyms, structures, source xrefs, spectra, evidence summaries, usage/commonness, provenance, and freshness when those fields are available locally.

## CLI examples

```bash
chemlake --data-dir ./lake resolve --input queries.txt --from comptox --to all --confidence
printf '%s
' 'DTXSID5020108' | chemlake --data-dir ./lake resolve --input - --to all --confidence
```

## API example

```json
{
  "queries": ["DTXSID5020108"],
  "from": "comptox",
  "to": "all",
  "confidence": true
}
```

## Notes

No additional caveats beyond local artifact availability.

## Updating this README

Do not edit this file directly. Edit `src/chem_evidence/routing_catalog.py`, then run `python scripts/generate_resolver_docs.py`.
