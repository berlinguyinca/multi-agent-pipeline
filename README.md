...
For chemical taxonomy-plus-usage prompts, MAP treats router outputs that merely explain why `researcher` was skipped in favor of specialized taxonomy/usage agents as a recoverable routing hint, not as a terminal no-match. The deterministic fallback routes to `classyfire-taxonomy-classifier` and `usage-classification-tree`, and reports suppress that implementation-detail `researcher` skip rationale once the specialist plan is selected.
